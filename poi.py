...
def search_welipro(query, search_type="name"):
    """
    Recherche sur Welipro et retourne les données de la société la plus proche.
    search_type : "ice", "rc" ou "name"
    """
    type_mapping = {
        "ice": "ice",
        "rc": "rc",
    ...
    # Cherche le nom le plus proche si recherche par nom
    if search_type == "name":
        ...
        if closest_name:
            for company in companies:
                if company.get('name') == closest_name[0]:
                    return company
    # Sinon, retourne le premier résultat
    return companies[0]


def enrich_clients_dataframe(df):
    """Enrichit un DataFrame avec les données Welipro."""
    enriched_rows = []

    for idx, row in df.iterrows():
        # Choisir la colonne pour la recherche : ICE > RC > Raison Sociale
        query = None
        search_type = "name"
...
# df_clients = pd.read_excel("clients.xlsx")  # ton DataFrame existant
df_clients = pd.DataFrame([
    {
        "numero_personne_host": 1,
        "raison_sociale": "NOVANCY CONSULTING",
        "sigle": "NC",
        "rc_and_ct": "265509",
        "ice": "000003602000037"
    },
    {
        "numero_personne_host": 2,
    ...
    {
        "numero_personne_host": 3,
        "raison_sociale": "TECHNOLOGIE MAROC",
        "sigle": "TM",
        "rc_and_ct": "",
        "ice": ""
    }
])
df_enriched = enrich_clients_dataframe(df_clients)
